 ...
 * @version 1.0
 */
public class CastlePowerSupplyLib implements ILib {
	/**
	 * Class Constants
	 * */
	 ...
	 * Class Constructors
	 **/
	public CastlePowerSupplyLib() {
		CastlePowerSupplyLib.psObj = this.exu().modTS().objPowerSupplyControl();
		CastlePowerSupplyLib.psDacObj = this.exu().modTS().objDAC();
		initSupplyParameters();
	}

	public CastlePowerSupplyLib(IExu exu) {
		initSupplyParameters();
		CastlePowerSupplyLib.exu = (ETS) exu;
		CastlePowerSupplyLib.psObj = this.exu().modTS().objPowerSupplyControl();
		CastlePowerSupplyLib.psDacObj = this.exu().modTS().objDAC();
	}
	...
	public ItfPowerSupplyControl getObj() {
		return CastlePowerSupplyLib.psObj;
	}
	 ...
	 */
	public ETS exu() {
		return CastlePowerSupplyLib.exu;
	}
	 ...
	 * @see {@link #getSupplyMaxVolt getSupplyMaxVolt"}
	 **/
	public void setChannelVoltExt(PowerSupplyID Channel, double Volt) {

		// Select proper DAC channel 
		EnmDacChnls dacChannel = EnmDacChnls.DAC_Ch_15;
		if (Channel.equals(PowerSupplyID.S2)){dacChannel = EnmDacChnls.DAC_Ch_16;}
	 ...
	 **/
	public void selectPwrSuppChannel(PowerSupplyID Chnl) {
	 ...
	 *            Voltage value in volts
	 */
	public void setSupplyType(PowerSupplyType Supp) {
		if (Supp.equals(PowerSupplyType.ZUP2020)) {
			this.maxVolt = ZUP2020_MAX_VOLT;
			this.maxCurrent = ZUP2020_MAX_CURRENT;
		} else if (Supp.equals(PowerSupplyType.TOELLNER4010)) {
			this.maxVolt = TOELLNER4010_MAX_VOLT;
			this.maxCurrent = TOELLNER4010_MAX_CURRENT;
		} else if (Supp.equals(PowerSupplyType.TOELLNER2020)) {
	 ...
	 **/
	public void setChannelOutput(EnmPowerSource enuState) {
	 ...
	 *      Example for setChannelOutput API}
	 **/
	public void setExternalMode(PowerSupplyID Channel, EnmPowerSource enuState) {
	...
	private EnmPowerSupplyOutput getState(EnmPowerSource state) {

		int index = Arrays.asList(EnmPowerSource.values()).indexOf(state);
